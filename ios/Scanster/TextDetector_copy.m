...
RCT_REMAP_METHOD(scan, scan:(id)beforeConvertImage :(double)x :(double)y :(double)width :(double)height
                 resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject) {
    
    // convert image from react native to UIImage
    UIImage *imageBeforeCrop = [RCTConvert UIImage:beforeConvertImage];
    
    // crop the image first
    UIImage *image = nil;
    CGRect cropRect = CGRectMake((CGFloat)x, (CGFloat)y, (CGFloat)width, (CGFloat)height);
    image = [self crop:imageBeforeCrop toRect:cropRect];
